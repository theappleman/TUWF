...
with the exception that (like all other TUWF methods) C<reqMethod()> returns
all data in Perls native unicode format and that for file uploads, the file
name is returned instead of its contents.
...
C<Content-Length> headers with POSTS requests may not be present, even when
they have been sent by the client.
...
address, depending on your webserver configuration.
