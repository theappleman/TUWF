...
many cases complicates the implementation of new ideas and may add bloat to the
framework. For this reason, future versions of TUWF may work differently from
older versions, and may not be backward compatible with code generated for an
older version.
...
functions. These functions are provided by several TUWF submodules (e.g.
...
=head2 log(message, exclude_info)
...
with the date and URI, but this can be disabled by setting I<exclude_info> to a
true value.
